...
Layout *AdvanceMapParser::parseLayout(const QString &filepath, bool *error, const Project *project)
{
    ...
    if (numBorderTiles == 0) {
        ...
        if (in.length() >= mapDataOffset + baseMapSize + 4) {
            ...
            int candidateBorderTiles = trailingWidth * trailingHeight;
            int candidateBorderSize = candidateBorderTiles * 2;
            int candidateBorderOffset = in.length() - (candidateBorderSize + 4);
            if (candidateBorderOffset >= mapDataOffset + baseMapSize) {
                borderWidth = trailingWidth;
                borderHeight = trailingHeight;
                numBorderTiles = candidateBorderTiles;
                baseBorderSize = candidateBorderSize;
                borderOffset = candidateBorderOffset;
